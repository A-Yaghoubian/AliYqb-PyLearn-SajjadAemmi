...
def zoom_effect(frame, landmarks) : # for eyes
    ...
    # frame_2x = cv2.resize(frame, None, fx=1.5, fy=1.5)
    # mask_2x = cv2.resize(mask, None, fx=1.5, fy=1.5)
    frame_2x = cv2.resize(frame, None, fx=2, fy=3)
    mask_2x = cv2.resize(mask, None, fx=2, fy=3)
    
    frame_2x = frame_2x / 255
    mask_2x = mask_2x / 255
    
    # frame_target = frame[int(y - (h * 0.25)) : int(y + h + (h * 0.25)), int(x - (w * 0.25)):int(x + w + (w * 0.25))]
    frame_target = frame[int(y - (h * 1)) : int(y + h + (h * 1)), int(x - (w * 0.5)):int(x + w + (w * 0.5))]
    frame_target = frame_target / 255
    
    # print((frame_target))
    # print((1 - mask_2x[int(y * 1.5) : int((y + h) * 1.5), int(x * 1.5) : int((x + w) * 1.5)]))
    
    foreground = cv2.multiply(mask_2x, frame_2x)
    # background = cv2.multiply(frame_target, 1 - mask_2x[int(y * 1.5) : int((y + h) * 1.5), int(x * 1.5) : int((x + w) * 1.5)])
    # res = cv2.add(background, foreground[int(y * 1.5) : int((y + h) * 1.5), int(x * 1.5) : int((x + w) * 1.5)])
    # frame[int(y- (0.25 * h)) : int(y + h + (0.25 * h)), int(x - (0.25 * w)) : int(x + w + (0.25 * w))] = res * 255
    background = cv2.multiply(frame_target, 1 - mask_2x[y * 3 : (y + h) * 3, x * 2 : (x + w) * 2])
    res = cv2.add(background, foreground[y * 3 : (y + h) * 3, x * 2 : (x + w) * 2])
    frame[int(y- (1 * h)) : int(y + h + (1 * h)), int(x - (0.5 * w)) : int(x + w + (0.5 * w))] = res * 255
    return frame
    
MIN_W = 300
MIN_H = 300
   
def advanced_zoom_effect(frame, landmarks, MIN_W, MIN_H) : # for lips
    x, y, w, h = cv2.boundingRect(landmarks)
    # print(x, y, w, h)
    ...
    if (7 < (w - MIN_W) <= 15 or 10 < (h - MIN_H) <= 20):
        Y = 2
        yy = 0.5
        X = 2
        xx = 0.5
        
    elif (15 < (w - MIN_W) <= 25 or 20 < (h - MIN_H) <= 45):
        Y = 3
        yy = 1
        X = 3
        xx = 1
        
    elif ((w - MIN_W) > 25 or (h - MIN_H) > 45):
        Y = 4
        yy = 1.5
        X = 4
        xx = 1.5
    ...
    frame_2x = cv2.resize(frame, None, fx=X, fy=Y)
    mask_2x = cv2.resize(mask, None, fx=X, fy=Y)
    
    frame_2x = frame_2x / 255
    mask_2x = mask_2x / 255
    
    frame_target = frame[int(y - (h * yy)) : int(y + h + (h * yy)), int(x - (w * xx)) : int(x + w + (w * xx))]
    frame_target = frame_target / 255
    
    foreground = cv2.multiply(mask_2x, frame_2x)
    
    try:
        background = cv2.multiply(frame_target, 1 - mask_2x[y * Y : (y + h) * Y, x * X : (x + w) * X])
        res = cv2.add(background, foreground[y * Y : (y + h) * Y, x * X : (x + w) * X])
        frame[int(y - (h * yy)) : int(y + h + (h * yy)), int(x - (w * xx)) : int(x + w + (w * xx))] = res * 255
    except:
        pass
    
    return frame, MIN_W, MIN_H

if __name__ == '__main__':
    fd = UltraLightFaceDetecion("E:\programming\PyLearn-SajjadAemmi\Assignment24\OpenVtuber\weights\RFB-320.tflite", conf_threshold=0.88)
    fa = CoordinateAlignmentModel("E:\programming\PyLearn-SajjadAemmi\Assignment24\OpenVtuber\weights\coor_2d106.tflite")

    # frame = cv2.imread("input/face.jpg")
    ...
    result = cv2.VideoWriter('output.avi', fourcc, 5, size)
    ...
    while True:
        ...
        for pred in fa.get_landmarks(frame, boxes):
            ...
            # cv2.drawContours(mask_left_eye, [landmarks_left_eye], -1, (255, 255, 255), -1)
            # cv2.drawContours(mask_right_eye, [landmarks_right_eye], -1, (255, 255, 255), -1)
            # cv2.drawContours(mask_lips, [landmarks_lips], -1, (255, 255, 255), -1)
            
            frame, MIN_W, MIN_H = advanced_zoom_effect(frame, landmarks_lips, MIN_W, MIN_H)
