...
while(True):
    ...
    # rec = cv2.equalizeHist(rec)   
    
    alpha = 1.5 
    beta = 0 
    rec = cv2.convertScaleAbs(rec, alpha=alpha, beta=beta)
    
    frame[180 : 300, 250 : 370] = rec
    
    if 0 <= np.average(rec) < 85:
        text = "black"
    elif 85 <= np.average(rec) < 170:
        text = "gray"
    elif 170 <= np.average(rec) <= 255:
        text = "white"
    
    cv2.putText(frame, text, (20, 50), cv2.FONT_HERSHEY_SCRIPT_COMPLEX, 2, (255, 255, 255), 1)
    cv2.putText(frame, str(int(np.average(rec))), (20, 90), cv2.FONT_HERSHEY_SCRIPT_COMPLEX, 0.8, (255, 255, 255), 1)
